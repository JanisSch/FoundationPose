...
class FileSelectorGUI:
    def __init__(self, master, file_paths):
        ...
        self.selected_paths = []  # Store the selected paths for tracking
        self.check_vars = []  # Store the variables for checkboxes

        # Add a label to guide the user
        self.label = tk.Label(master, text="Select objects to track by checking the boxes:")
        ...
        for file_path in self.file_paths:
            ...
            checkbox = tk.Checkbutton(self.checkbox_frame, text=file_name, variable=var)
    ...
    def done(self):
        """Save the selected paths and close the GUI."""
        self.selected_paths = [file_path for var, file_path in self.check_vars if var.get()]
        self.master.quit()

    def get_selected_paths(self):
        """Return the selected file paths for tracking after the GUI has closed."""
...
def rearrange_files(file_paths):
    ...
    selected_paths = app.get_selected_paths()  # Get selected paths after GUI closes
...
class PoseEstimationNode(Node):
    def __init__(self, selected_file_paths):
        ...
        self.mesh_files = selected_file_paths
...
def main(args=None):
    ...
    selected_file_paths = rearrange_files(file_paths)

    rclpy.init(args=args)
    node = PoseEstimationNode(selected_file_paths)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
