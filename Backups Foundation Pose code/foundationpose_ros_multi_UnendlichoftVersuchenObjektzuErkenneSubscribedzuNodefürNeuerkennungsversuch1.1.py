...
class PoseEstimationNode(Node):
    def __init__(self, selected_objects):
        ...
        # Subscribe to the /redo topic
        self.redo_sub = self.create_subscription(Bool, '/redo', self.redo_callback, 10)
    ...
    def redo_callback(self, msg):
        ...
        if msg.data:  # If the message data is True
            self.get_logger().info("Redo signal received. Resetting initial detection.")
    ...
    def process_images(self):
        ...
        if not self.initial_detection_done:
            # Perform object detection and registration until at least one valid object is detected
            self.get_logger().info("Performing object detection and registration.")
            while not self.pose_estimations:  # Ensure at least one object is registered
                detected_objects = []
                ...
                if not detected_objects:
                    self.get_logger().warn("No valid objects detected. Waiting for the next image.")
                    rclpy.spin_once(self, timeout_sec=0.1)  # Wait for the next image
                    continue
                ...
                if not self.pose_estimations:
                    self.get_logger().warn("No objects successfully registered. Retrying detection.")
